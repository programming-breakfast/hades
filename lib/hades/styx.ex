defmodule Hades.Styx do
  ...
  def init(_) do
    config = [
      %Soul{name: "foo", start: "while true; do sleep 100; done & echo $! > %pid_file%", pid_file: "tmp/foo.pid"},
      %Soul{name: "bar", start: "while true; do sleep 200; done & echo $! > %pid_file%", stop: "kill -9 `cat tmp/bar.pid`", pid_file: "tmp/bar.pid"},
